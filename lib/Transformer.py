...
class OptimalTransport():
    ...
    def transport(self,S,T,toComputeCost=True,costType='sq_l2',normalizeCost=True,costScale=100,transformType='basic', 
        fair=False,alpha_s=1,is_prot_s=np.ones(5),alpha_t=1,is_prot_t=np.ones(5),toSaveTransport=False,transportPath='/'):
    # def transform(self,S,T,toComputeCost=True,costType='sq_l2',normalizeCost=False,transformType='basic', fair=False,target_marginal=np.ones(5)):
        error = 0
        error, S = self._cleanInput(S)
        error, T = self._cleanInput(T)
    ...
    #     # print(f'loss shape: {loss.shape}')
    #     return(loss)
    def compute_cuturi_sinkhorn_loss(self):
        n_s = self.P.shape[0]
        # mask = mask[:n_s]
        # masked_P = self.P[mask,:]
        # masked_C = self.C[mask,:]
        ...
        # print(f'mask shape: {mask.shape}')
        m = torch.mul(self.P,self.C)
        entropy = self.l*torch.mul(self.P,torch.log(self.P))# -1*lambda*entropy (since entropy negative we ignore first -1)
    ...
    #     # print(f'loss shape: {loss.shape}')
    #     return(loss)
    def compute_fair_sinkhorn_loss(self):
        n_s = self.P.shape[0]
        # mask = mask[:n_s]
        # masked_P = self.P[mask,:]
        # masked_C = self.C[mask,:]
        # masked_R = self.R_prot[mask,:]
        # masked_S = self.S_prot[mask,:]
        n_r = torch.sum(torch.sum(self.R_prot))
        n_s = torch.sum(torch.sum(self.S_prot))
        # print(f'mask shape: {mask.shape}')
        m = torch.mul(self.P,self.C)
        entropy = self.l*torch.mul(self.P,torch.log(self.P))# -1*lambda*entropy (since entropy negative we ignore first -1)

        fair = (self.R_prot/n_r) - (self.S_prot/n_s)
        fair = self.g*torch.mul(self.P,fair)
