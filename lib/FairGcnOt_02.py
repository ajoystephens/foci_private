...
class ModelWrapper():
    ...
    def load_and_split_source(self,source,fold_assignments, fold,ppmi=False):
        self.source = source
        ...
        if ppmi:
            PPMI = agg_tran_prob_mat(A, self.ppmi_step)
            PPMI = compute_ppmi(PPMI)
            A = sp.lil_matrix(PPMI)
        else:
            A = A.tolil()
    ...
    def train_ot(self, to_save_loss=False, to_save_hidden=False):
        ...
        for epoch in range(self.options['max_ot_epochs']):
            ...
            ot_loss = self.model.get_transport_loss(train_mask, fair=True)
            ...
            if self.to_validate:
                ...
                with torch.no_grad():
                    ...
                    val_ot_loss = self.model.get_transport_loss(val_mask,fair=True)
