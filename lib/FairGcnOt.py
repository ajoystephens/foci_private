...
class ModelWrapper():
    ...
    def pretrain(self, to_save_loss=False):
        ...
        for epoch in range(self.options['max_pretrain_epochs']+1):
            ...
            if self.to_validate and (len(val_losses)>5):
                check = val_losses[-10:]
                if all(element == check[0] for element in check): break # stop if all elements are the same
                trend = linregress(check,range(len(check))).slope
                trends.append(trend)
                if min(trends[-5:])>0:break
    ...
    def test(self,to_save_hidden=False):
        ...
        p0_y = y_test[P_t==0]
        p0_pred = pred_test[P_t==0]
        self.log.info(f'Prot=0 CM:\n{confusion_matrix(p0_y,p0_pred)}')

        p1_y = y_test[P_t==1]
        p1_pred = pred_test[P_t==1]
        self.log.info(f'Prot=1 CM:\n{confusion_matrix(p1_y,p1_pred)}')
