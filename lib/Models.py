...
class OTGCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index,to_transform=False,to_save_transform=False,transform_path='/',to_fair=True):
    ...
    #     return(loss)
    def get_transport_loss(self, fair):
        # loss = self.transformer.compute_wass_loss(mask)
        if fair: loss = self.transformer.compute_fair_sinkhorn_loss()
        else: loss = self.transformer.compute_cuturi_sinkhorn_loss()
        return(loss)